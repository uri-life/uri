...
Describe 'general-purpose Git patchset E2E'
  ...
  setup_generic_patchset() {
    ...
    git -C "$GENERIC_SOURCE" tag "v1.2.3+build"
    ...
    cmd_init --upstream "$GENERIC_SOURCE" --branch-prefix custom \
      ...
      "v1.2.3+build" >/dev/null
    URI_ROOT="$GENERIC_ROOT"
    export URI_ROOT
    cmd_add "v1.2.3+build" "stack-a" >/dev/null
    cmd_add "v1.2.3+build" "stack-a" "feature-a" >/dev/null
    git -C "$GENERIC_SOURCE" format-patch --stdout "v1.2.3+build..HEAD" > \
      "$GENERIC_ROOT/versions/v1.2.3+build/patches/stack-a/feature-a.patch"

    cmd_add "v1.2.3+build" "next-stack" --inherits "stack-a" >/dev/null
    cmd_add "v1.2.3+build" "next-stack" "feature-b" --dependencies "feature-a" >/dev/null
  }
  BeforeEach 'setup_generic_patchset'

  auto_apply_flow() {
    _e2e_output=$(cmd_apply "v1.2.3+build" "stack-a" 2>&1) || {
      printf '%s\n' "$_e2e_output"
      return 1
    }
    ...
    git_branch_exists "$_e2e_destination" "custom/v1.2.3+build/stack-a" || return 1
  ...
  It 'apply without a destination preserves a full clone and uses a custom prefix and explicit committer'
    ...
    The output should include 'custom/v1.2.3+build/stack-a'
    The output should include 'verified destination:'
  End

  auto_expand_collapse_flow() {
    _e2e_output=$(cmd_expand "v1.2.3+build" "stack-a" "feature-a" 2>&1) || {
      printf '%s\n' "$_e2e_output"
      return 1
    }
    _e2e_destination=$(printf '%s\n' "$_e2e_output" | sed -n 's/^info: Automatically created destination: //p' | tail -n 1)
    [ -d "$_e2e_destination/.git" ] || return 1
    git_branch_exists "$_e2e_destination" "custom/v1.2.3+build/stack-a/feature-a" || return 1
    cmd_collapse "v1.2.3+build" "stack-a" "feature-a" "$_e2e_destination" >/dev/null || return 1
    ! git_branch_exists "$_e2e_destination" "custom/v1.2.3+build/stack-a/feature-a" || return 1
    [ -s "$GENERIC_ROOT/versions/v1.2.3+build/patches/stack-a/feature-a.patch" ] || return 1
    printf '%s\n' "$_e2e_output"
  ...
  repository_committer_flow() {
    ...
    cmd_apply "v1.2.3+build" "stack-a" "$_e2e_destination" >/dev/null || return 1
  ...
  legacy_manifest_flow() {
    ...
    git clone --quiet "$GENERIC_SOURCE" "$_e2e_destination" || return 1
    cmd_apply "v1.2.3+build" "stack-a" "$_e2e_destination" >/dev/null || return 1
    git_branch_exists "$_e2e_destination" "uri/v1.2.3+build/stack-a" || return 1
    git -C "$_e2e_destination" log -1 --format='%cn <%ce>'
  }
  ...
  missing_repository_identity_is_unchanged() {
    ...
    (
      ...
      cmd_apply "v1.2.3+build" "stack-a" "$_e2e_destination" >/dev/null 2>&1
    ) && return 1
  ...
  It 'resolves an inherited non-uri patchset in list'
    When call cmd_list "v1.2.3+build" "next-stack"
  ...
  It 'prints the graph of an inherited non-uri patchset'
    When call cmd_graph "v1.2.3+build" "next-stack"
    The status should be success
    The output should include 'feature-a'
    The output should include 'feature-b'
  End
  ...
  origin_mismatch_is_unchanged() {
    ...
    _e2e_patch=$(shasum -a 256 "$GENERIC_ROOT/versions/v1.2.3+build/patches/stack-a/feature-a.patch" | awk '{print $1}')

    if (cmd_apply "v1.2.3+build" "stack-a" "$_e2e_destination" >/dev/null 2>&1); then
      return 1
    fi

    [ "$(git -C "$_e2e_destination" rev-parse HEAD)" = "$_e2e_head" ] || return 1
    [ "$(git -C "$_e2e_destination" for-each-ref --format='%(refname):%(objectname)' refs/heads)" = "$_e2e_refs" ] || return 1
    [ "$(shasum -a 256 "$GENERIC_ROOT/versions/v1.2.3+build/patches/stack-a/feature-a.patch" | awk '{print $1}')" = "$_e2e_patch" ] || return 1
    ! state_exists "$_e2e_destination"
  }
  ...
  invalid_identifier_is_unchanged() {
    ...
    git clone --quiet "$GENERIC_SOURCE" "$_e2e_destination" || return 1
    _e2e_head=$(git -C "$_e2e_destination" rev-parse HEAD)
    if (cmd_apply "bad/version" "stack-a" "$_e2e_destination" >/dev/null 2>&1); then
      return 1
    fi
    [ "$(git -C "$_e2e_destination" rev-parse HEAD)" = "$_e2e_head" ] || return 1
    [ "$(git -C "$_e2e_destination" for-each-ref --format='%(refname)' refs/heads)" = "refs/heads/main" ] || return 1
